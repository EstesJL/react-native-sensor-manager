   ...
   // float mOrientationData[] = new float[3];
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
      ...
      if (mySensor.getType() == Sensor.TYPE_ACCELEROMETER) {
        mAcc = lowPass(sensorEvent.values.clone(), mAcc);
      ...
      if (mySensor.getType() == Sensor.TYPE_GRAVITY) {
        mGrav = sensorEvent.values.clone();
      ...
      if (mAcc != null && mGeomagnetic != null) {
        float R[] = new float[9];
        float I[] = new float[9];
        boolean success = mSensorManager.getRotationMatrix(R, I, mAcc, mGeomagnetic);
        if (success) {
          ...
          //
          float heading = (float)(orientation[0]);
          float pitch = (float)(orientation[1]);
          float roll = (float)(orientation[2]);

          if (mGrav != null) {
              map.putDouble("gravityX", mGrav[0]);
          }
          ...
          // float heading = (float)((Math.toDegrees(mOrientationData[0])));
          // float pitch = (float)((Math.toDegrees(mOrientationData[1])));
          // float roll = (float)((Math.toDegrees(mOrientationData[2])));
